...
public:
    ...
    bool alive(Entity entity) const {
        return m_alive.contains(entity.id());
    }
    ...
    public:
        ...
        void each(Func&& func) {
            auto* first_storage = m_world.get_storage<std::tuple_element_t<0, std::tuple<Components...>>>();
            if (!first_storage) return;
...
private:
    ...
    ComponentStorage<T>& get_or_create_storage() {
        ...
        if (it == m_storages.end()) {
            ...
            m_component_removers[type] = [this](EntityID id) {
                if (auto* s = get_storage<T>()) s->remove(id);
            };
        }
    ...
    std::unordered_map<std::type_index, std::function<void(EntityID)>> m_component_removers;
};
