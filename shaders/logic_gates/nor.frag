...
void main() {
    // NOR gate: OR body + inversion bubble, scaled to [-1, 1]
    float d = sdBezier(uv, vec2(-1.0, 1.0), vec2(-0.5, 0.0), vec2(-1.0, -1.0));
    d = min(d, sdBezier(uv, vec2(-1.0, 1.0), vec2(0.2, 1.0), vec2(0.7, 0.0)));
    d = min(d, sdBezier(uv, vec2(-1.0, -1.0), vec2(0.2, -1.0), vec2(0.7, 0.0)));

    // Inversion bubble at right edge
    float dotDist = abs(length(uv - vec2(0.85, 0.0)) - 0.15);
